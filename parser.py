...
def is_maintenance_email(self, email_data: Dict) -> bool:
    ...
    ИСПРАВЛЕННАЯ ВЕРСИЯ с расширенным списком ключевых слов
    ...
    try:
        ...
        # РАСШИРЕННЫЕ ключевые слова для определения уведомлений о работах
        keywords = [
            # Плановые работы
            'плановые работы',
            'технических работ',
            ...
            # Email2SMS специфичные (ДОБАВЛЕНО!)
            'email2sms',
            'проблемы с доставкой',
            'доставка сообщений',
            'sms доставка',

            # Платформа Devino (ДОБАВЛЕНО!)
            'на платформе devino',
            'платформа devino telecom',
            'сервисы платформы',
            'работа платформы'
        ]
        ...
        self.logger.debug(f"Проверка письма: {subject[:50]}...")
        ...
        # Дополнительная проверка: если отправитель Devino и есть упоминание работ/сбоев
        email_from = email_data.get('from', '').lower()
        if 'devino' in email_from or 'devinotele' in email_from:
            devino_keywords = [
                'работы',
                'работ',
                'сбой',
            ...
            for keyword in devino_keywords:
                if keyword in full_text:
                    self.logger.debug(f"✅ Devino письмо с ключевым словом: '{keyword}'")
                    return True

        self.logger.debug(f"❌ Ключевые слова не найдены")
        return False
